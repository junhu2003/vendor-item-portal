  ...
  try {
    const invoices = await sql.query<InvoicesTable[]>`
      ...
      WHERE
        ...
        CAST(invoices.amount AS VARCHAR(50)) LIKE ${`%${query}%`} OR
        CAST(invoices.date AS VARCHAR(100)) LIKE ${`%${query}%`} OR
...
export async function fetchInvoicesPages(query: string) {
  try {
    const data = await sql.query`SELECT COUNT(*) AS total
    FROM invoices
    JOIN customers ON invoices.customer_id = customers.id
    WHERE
      customers.name LIKE ${`%${query}%`} OR
  ...
  `;
    ...
    const totalPages = Math.ceil(data.recordset[0].total / ITEMS_PER_PAGE);
    return totalPages;
  } catch (error) {
    console.error('Database Error:', error);
